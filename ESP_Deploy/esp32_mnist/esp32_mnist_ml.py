# esp32_mnist_ml.py -- Export all ML models for MNIST digits (sklearn 8x8)
import numpy as np
...
CLASS_NAMES = ['"Digit_{}"'.format(i) for i in range(10)]
...
def export_common():
    ...
    lines = [
        "// common_model.h -- MNIST digits (8x8)",
        "#pragma once",
        "#include <stdint.h>",
        "#include <pgmspace.h>",
        "",
...
def export_knn():
    ...
    # Digits pixel values 0-16, QSCALE=10000 → max_q=160000 > 32767
    if max_q > 32767:
    ...
    lines = [
        "// knn_model.h -- KNN (MNIST digits 8x8)",
        "#pragma once",
...
def export_dt():
    ...
    lines = [
        "// dt_model.h -- Decision Tree (MNIST digits)",
        "#pragma once",
        '#include "common_model.h"',
        "",
...
def export_rf():
    ...
    rf = RandomForestClassifier(
        n_estimators=10, max_depth=8, random_state=42
    )
    ...
    lines = [
        "// rf_model.h -- Random Forest (MNIST digits)",
        "#pragma once",
        '#include "common_model.h"',
        "",
...
def export_svm():
    ...
    # SVM_CLF_N_SV may exceed uint8_t for 10-class, use uint16_t
    max_clf_nsv = max(clf_n_sv)
    nsv_type = "uint8_t" if max_clf_nsv <= 255 else "uint16_t"

    lines = [
        "// svm_model.h -- SVM OvO (MNIST digits)",
        "#pragma once",
        '#include "common_model.h"',
        "",
    ...
    lines += [
        "};",
        "",
        "static const " + nsv_type + " SVM_CLF_N_SV[SVM_N_CLF] PROGMEM = {",
