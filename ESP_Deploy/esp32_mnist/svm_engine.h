// svm_engine.h -- SVM OvO engine (MNIST digits)
#pragma once
#include <Arduino.h>
#include <pgmspace.h>
#include <math.h>
...
IRAM_ATTR static float rbf_kernel_single(const int16_t* q, uint16_t sv_idx) {
    float sq = 0.0f;
...
IRAM_ATTR MLResult svm_predict(const float* raw) {
    int16_t q[ML_N_FEAT];
    scale_and_quantize(raw, q, ML_SCALER_MIN, ML_SCALER_RANGE, ML_N_FEAT, ML_QSCALE);

    uint32_t t0 = ccount();

    // Allocate K on heap since SVM_N_SV can be large for 10-class
    float* K = (float*)malloc(SVM_N_SV * sizeof(float));
    if (!K) { MLResult r = {0, 0.0f, 0, 0.0f}; return r; }
    ...
    for (uint8_t clf = 0; clf < SVM_N_CLF; clf++) {
        ...
        for (uint16_t k = 0; k < ns; k++) {
            uint16_t sv_idx = (uint16_t)(int16_t)pgm_read_word(&SVM_CLF_SV_IDX[off + k]);
    ...
    MLResult r;
    r.label = best; r.confidence = conf; r.cycles = cyc; r.us = cycles_to_us(cyc);
    return r;
}
