// mlp_engine.h -- MLP engine (MNIST digits)
#pragma once
#include <Arduino.h>
#include <pgmspace.h>
#include <math.h>
...
IRAM_ATTR MLResult mlp_predict(const float* raw) {
    ...
    scale_and_quantize(raw, q16, ML_SCALER_MIN, ML_SCALER_RANGE, ML_N_FEAT, ML_QSCALE);
    ...
    float e[ML_N_CLASSES], sum_e = 0.0f;
...
void mlp_eval() {
    eval_testset_scaled("MLP 32-32-10", mlp_predict, ML_CLASS_NAMES);
}
