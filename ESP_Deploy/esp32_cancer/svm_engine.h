// svm_engine.h -- SVM OvO engine (Breast Cancer)
#pragma once
#include <Arduino.h>
#include <pgmspace.h>
#include <math.h>
...
IRAM_ATTR MLResult svm_predict(const float* raw) {
    ...
    MLResult r; r.label = best; r.confidence = conf;
    r.cycles = cyc; r.us = cycles_to_us(cyc); return r;
}

void svm_eval() { eval_testset_scaled("SVM RBF", svm_predict, ML_CLASS_NAMES); }
