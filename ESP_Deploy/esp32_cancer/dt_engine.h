// dt_engine.h -- Decision Tree engine (Breast Cancer)
#pragma once
#include <Arduino.h>
#include <pgmspace.h>
#include "utils.h"
...
IRAM_ATTR static int dt_traverse(const int32_t* q) {
    int16_t node = 0; uint16_t guard = 0;
    while (guard++ < (uint16_t)(DT_N_NODES + 5)) {
        int8_t feat = pm_i8(&DT_FEATURE[node]);
        if (feat < 0) return (int)pm_i8(&DT_CLASS[node]);
...
IRAM_ATTR MLResult dt_predict(const float* raw) {
    ...
    MLResult r; r.label = (uint8_t)label; r.confidence = 100.0f;
    r.cycles = cyc; r.us = cycles_to_us(cyc); return r;
}

void dt_eval() { eval_testset_raw("Decision Tree", dt_predict, ML_CLASS_NAMES); }
