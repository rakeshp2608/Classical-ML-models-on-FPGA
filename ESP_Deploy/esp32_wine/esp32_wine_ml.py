# esp32_wine_ml.py -- Export all ML models for Wine dataset
import numpy as np
...
def export_knn():
    ...
    # Check if raw values fit in int16 with QSCALE
    max_raw = np.max(np.abs(X_tr_raw))
    max_q = int(max_raw * QSCALE)
    print("  max raw value: {:.2f}, max q: {}".format(max_raw, max_q))

    if max_q > 32767:
        # Wine has values up to ~1700 (proline), so QSCALE=10000 is too much
    ...
    lines = [
        "// knn_model.h -- KNN on RAW data (Wine)",
        "#pragma once",
