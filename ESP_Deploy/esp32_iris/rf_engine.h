// rf_engine.h -- FIXED: leaf check via feature < 0
#pragma once
#include <Arduino.h>
#include <pgmspace.h>
#include "utils.h"
...
IRAM_ATTR MLResult rf_predict(const float* raw) {
    ...
    for (uint8_t t = 0; t < RF_N_TREES; t++) {
        ...
        if (pred >= 0 && pred < (int)ML_N_CLASSES)
            votes[(uint8_t)pred]++;
    }
...
void rf_eval() {
    eval_testset_raw("Random Forest", rf_predict, ML_CLASS_NAMES);
}