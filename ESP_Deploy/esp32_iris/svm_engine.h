// svm_engine.h -- FIXED: proper OvO binary classifiers
#pragma once
#include <Arduino.h>
#include <pgmspace.h>
#include <math.h>
...
IRAM_ATTR MLResult svm_predict(const float* raw) {
    ...
    // Precompute K(x, sv_i) for all SVs
    float K[SVM_N_SV];
    for (uint16_t i = 0; i < SVM_N_SV; i++)
        K[i] = rbf_kernel_single(q, i);

    // OvO: for each binary classifier compute decision value
